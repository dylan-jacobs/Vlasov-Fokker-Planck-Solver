...
% DTvals = [0.8, 0.4, 0.2, 0.1];
% DTvals = [0.5, 0.25, 0.125, 0.0625];
% DTvals = [0.001]; % for reference
% DTvals = [0.4];
DTvals = 0.2:0.02:0.9;
% DTvals = 0.2;
Nx = 160;
...
ERRORS(1, dtIndex) = dx*sum(error)/(200*16*8);
ERRORS(2, dtIndex) = dx*sum(abs(n_vals - soln_n))/200;
...
% for j = 1:Nx
%     ERRORS(6 + j, dtIndex) = 2*pi*dr*dz*sum(sum(abs(f_vals_low_rank{j, 1}*f_vals_low_rank{j, 2}*f_vals_low_rank{j, 3}' - soln{j, 1}*soln{j, 2}*soln{j, 3}')));
% end
end
...
% loglog(DTvals, 9e-3*(DTvals .^ 1), '--', 'Color', c_blue, 'LineWidth', 1.5); 
...
loglog(DTvals, 2e-4*(DTvals .^ 2), '--', 'Color', c_gray, 'LineWidth', 1.5);

% loglog(DTvals, 2.5e-5*(DTvals .^ 3), '--', 'Color', c_green, 'LineWidth', 1.5);
...
PlotF(soln, Xmat, Rmat, Zmat2, Nz, 11);
...
% disp(ERRORS(6, :)');
% disp(log2(ERRORS(6, 1:end-1)./ERRORS(6, 2:end))');
