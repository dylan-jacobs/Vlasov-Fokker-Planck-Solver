...
for dt_index = 1:numel(DTvals)
    ...
    % discretize time
    tvals = [0, 5e-3:dt:tf]';
    % tvals = [0:dt:tf]'; % for reference soln only
    if tvals(end) ~= tf
        tvals = [tvals; tf];
    end
    Nt = numel(tvals);
    ...
    momentum = zeros(Nt, 1);
    energy = zeros(Nt, 1);
    min_vals = zeros(Nt, numel(methods));
    avg_ranks = zeros(Nt, numel(methods));
    Efield = zeros(Nt, numel(methods));
    ...
    for method_index = 1:numel(methods)
        ...
        % Fokker-Planck collision parameters
        Ar = @(u, w, t) w; % evaluated at cell centers
        ...
        for spatialIndex = 1:Nx
            ...
            r0 = min(r0, size(Vr, 2)); % initial rank
        ...
        min_vals(1, method_index) = min(spatial_min_vals);
        avg_ranks(1, method_index) = mean(spatial_rank_vals);
        Efield(1, method_index) = sqrt(dx*sum(abs(spatial_Efield) .^ 2));
        ...
        for t_index = 2:Nt
            ...
            switch(method)
                case '1' % IMEX111
                    ...
                    parfor spatialIndex = 1:Nx
                        ...
                        [Vr, S, Vz, rank, E] = IMEX111(Vr, S, Vz, u_perp, u_para, dt, dx, tval, rvals, zvals, x_min, x_max, f_vals_low_rank, Rmat, Zmat, ma, me, qa, qe, Ar, Az, Br, Bz, Cr, Cz, tolerance, n_vals, Ta_vals, Te_vals, rhoM, JzM, kappaM, spatialIndex, R_const, leftBC, rightBC, wr, wz, c, w_norm_1_squared, w_norm_v_squared, w_norm_v2_squared);
                ...
                case '2' % IMEX222
                    ...
                    flux_diff_nu = flux_diff_nu...
                        ...
                        - dt*(gamma)*(qa/(2*qe*ma))*(n_IC(x_max + dx/2)*Te_IC(x_max + dx/2) + n_vals2(end)*Te_vals2(end) - n_vals2(1)*Te_vals2(1) - n_IC(x_min - dx/2)*Te_IC(x_min - dx/2));
                    flux_diff_nU = flux_diff_nU ...
            ...
            avg_ranks(t_index, method_index) = mean(spatial_rank_vals);
            min_vals(t_index, method_index) = min(spatial_min_vals);
            Efield(t_index, method_index) = sqrt(dx*sum(abs(spatial_Efield) .^ 2));
...
ylim([0, 1.2]);
legend('n', 'nu_{||}', 'T_e', 'T_a', 'Location', 'east');
...
plot(tvals, abs(momentum-momentum(1))/momentum(1), 'LineWidth', 1.5);
...
legend('Relative mass deviation', 'Relative momentum deviation', 'Relative energy deviation', 'Location', 'northeast');
fontsize(18,"points");
set(gcf,'Units','pixels','Position', [100 100 800 500]);
saveas(gcf, './Plots/VFP_structure_conservation.fig');
exportgraphics(gcf,'./Plots/VFP_structure_conservation.pdf','ContentType','vector');

% Rank plot
figure(4); clf;
plot(tvals, avg_ranks(:, 1), 'black-', 'LineWidth', 1.5); hold on;
plot(tvals, avg_ranks(:, 2), 'blue-', 'LineWidth', 1.5);
xlabel('time'); ylabel('Average rank'); % title('Average rank plot');
ylim([0, 10]);
legend('Backward Euler', 'IMEX222');
fontsize(18,"points");
set(gcf,'Units','pixels','Position', [100 100 800 500]);
saveas(gcf, './Plots/VFP_rank_plot.fig');
exportgraphics(gcf,'./Plots/VFP_rank_plot.pdf','ContentType','vector')

% Positivity
figure(5); clf; 
plot(tvals, min_vals(:, 1), 'black-', 'LineWidth', 1.5); hold on;
plot(tvals, min_vals(:, 2), 'blue-', 'LineWidth', 1.5);
xlabel('time'); ylabel('min(f)'); % title('Minimum values of numerical solution over time');
legend('Backward Euler', 'IMEX222', 'Location', 'southeast');
fontsize(18,"points");
set(gcf,'Units','pixels','Position', [100 100 800 500]);
saveas(gcf, './Plots/VFP_min_values.fig');
exportgraphics(gcf,'./Plots/VFP_min_values.pdf','ContentType','vector')


% Electric field
figure(6); clf;
semilogy(tvals, Efield(:, 1), 'black-', 'LineWidth', 1.5); hold on;
semilogy(tvals, Efield(:, 2), 'blue-', 'LineWidth', 1.5);
xlabel('time'); ylabel('||E||_2'); % title('L_2 norm of Electric Field over Time');
legend('Backward Euler', 'IMEX222', 'Location', 'southeast');
fontsize(18,"points");
set(gcf,'Units','pixels','Position', [100 100 800 500]);
saveas(gcf, './Plots/VFP_electric_field.fig');
exportgraphics(gcf,'./Plots/VFP_electric_field.pdf','ContentType','vector')


% Numerical solution
PlotF(f_vals_low_rank, Xmat, Rmat, Zmat2, Nz, 7); % title(sprintf('Numerical solution at time t=%s', num2str(tf)));
fontsize(18,"points");
set(gcf,'Units','pixels','Position', [100 100 800 500]);
saveas(gcf, './Plots/VFP_numerical_solution.fig');
exportgraphics(gcf,'./Plots/VFP_numerical_solution.pdf','ContentType','vector')

% Exact solution
PlotF(soln, Xmat, Rmat, Zmat2, Nz, 8); % title(sprintf('Exact solution at time t=%s', num2str(tf)));
fontsize(18,"points");
set(gcf,'Units','pixels','Position', [100 100 800 500]);
% saveas(gcf, './Plots/VFP_exact_solution.fig');
% exportgraphics(gcf,'./Plots/VFP_exact_solution.pdf','ContentType','vector')
...
function [Vr, S, Vz, rank, E] = IMEX111(Vr0, S0, Vz0, ur, uz, dt, dx, tval, rvals, zvals, x_min, x_max, f_vals_low_rank, Rmat, Zmat, ma, me, qa, qe, Ar, Az, Br, Bz, Cr, Cz, tolerance, n_vals, Ta_vals, Te_vals, rhoM, JzM, kappaM, spatialIndex, R_const, leftBC, rightBC, wr, wz, c, w_norm_1_squared, w_norm_v_squared, w_norm_v2_squared)
    dr = rvals(2) - rvals(1);
    dz = zvals(2) - zvals(1);
    Nr = numel(rvals);
    Nz = numel(zvals);
