...
Nx = 160;
Nvvals = [48, 80, 144, 240]; % chose these because these are odd factors of 720
% Nvvals = 720; % ref soln
ERRORS = zeros(5, numel(Nvvals));
dt = 0.2;
tf = 1;
...
% save('vdfp_refsoln_v2_Nx160_Nvperp720_Nvpara200_T1_tole-8_dt2e-1-imex222.mat','f_vals_low_rank');
% return
...
offset_start_idx = ceil(720/(2*Nr));
offset_end_idx = floor(720/(2*Nr));
offset_increment = 720/Nr;

for i = 1:Nx
    f = f_vals_low_rank{i, 1}*f_vals_low_rank{i, 2}*f_vals_low_rank{i, 3}';
    [~, Rmat_ref, Zmat_ref, dx_ref, dr_ref, dz_ref] = GetRZ(Nx, 720, 200, interval);
    f_exact = soln{i, 1}*soln{i, 2}*soln{i, 3}';

    soln_n(i) = 2*pi*dr_ref*dz_ref*sum(sum(f_exact.*Rmat_ref));
    soln_u(i) = (1/soln_n(i))*2*pi*dr_ref*dz_ref*sum(sum(Zmat_ref.*f_exact.*Rmat_ref));
    ...
    f_exact = f_exact(offset_start_idx:offset_increment:end-offset_end_idx, :);
    error(i) = 2*pi*dr*dz*sum(sum(abs(f - f_exact) .* Rmat));
    linf_error = max(linf_error, max(max(abs(f - f_exact))));

end
ERRORS(1, nvIndex) = dx*sum(error)/(200*16*8);
ERRORS(2, nvIndex) = dx*sum(abs(n_vals - soln_n))/200;
ERRORS(3, nvIndex) = dx*sum(abs(u_para - soln_u))/200;
ERRORS(4, nvIndex) = dx*sum(abs(Ta_vals - soln_Ta))/200;
ERRORS(5, nvIndex) = linf_error;
end
...
[~, Rmat_ref, Zmat_ref, dx_ref, dr_ref, dz_ref] = GetRZ(Nx, 720, 200, interval);
zvals_ref = Zmat_ref(1, :)';
[Xmat_ref, Zmat_ref2] = meshgrid(xvals, zvals_ref); Xmat_ref=Xmat_ref'; Zmat_ref2=Zmat_ref2'; % for plotting only


PlotF(soln, Xmat_ref, Rmat_ref, Zmat_ref2, dr_ref, 200, 11);
title(sprintf('Exact solution at time t=%s', num2str(tf)));


PlotF(f_vals_low_rank, Xmat, Rmat, Zmat2, dr, Nz, 10);
title(sprintf('Numerical solution at time t=%s', num2str(tf)));

disp('Errors:');
disp('L1 (global)');
disp(ERRORS(1, :)');
disp(log2(ERRORS(1, 1:end-1)./ERRORS(1, 2:end))' ./ log2(Nvvals(2:end) ./ Nvvals(1:end-1))');
disp('L_{inf} (global)');
disp(ERRORS(5, :)');
disp(log2(ERRORS(5, 1:end-1)./ERRORS(5, 2:end))'./  log2(Nvvals(2:end) ./ Nvvals(1:end-1))');
disp('Mass');
disp(ERRORS(2, :)');
disp(log2(ERRORS(2, 1:end-1)./ERRORS(2, 2:end))' ./ log2(Nvvals(2:end) ./ Nvvals(1:end-1))');
disp('Momentum');
disp(ERRORS(3, :)');
disp(log2(ERRORS(3, 1:end-1)./ERRORS(3, 2:end))' ./ log2(Nvvals(2:end) ./ Nvvals(1:end-1))');
disp('Energy');
disp(ERRORS(4, :)');
disp(log2(ERRORS(4, 1:end-1)./ERRORS(4, 2:end))' ./ log2(Nvvals(2:end) ./ Nvvals(1:end-1))');
...
% 
% figure; clf;
% loglog(NzVals, ERRORS2(1, :), '-', 'Color', c_blue, 'LineWidth', 1.5); hold on; % L1
% % loglog(NzVals, ERRORS(5, :), '-', 'Color', c_purple,  'LineWidth', 1.5); hold on; % Linf
% % loglog(NzVals, 9e-3*(NzVals .^ 1), '--', 'Color', c_blue, 'LineWidth', 1.5); 
% loglog(NzVals, ERRORS2(2, :), '-', 'Color', c_orange, 'LineWidth', 1.5); hold on; % nvals
% loglog(NzVals, ERRORS2(3, :), '-', 'Color', c_green,  'LineWidth', 1.5); hold on; % uvals
% loglog(NzVals, ERRORS2(4, :), '-', 'Color', c_red,  'LineWidth', 1.5); hold on; % Tvals
% loglog(NzVals, 2e-4*(NzVals .^ 2), '--', 'Color', c_gray, 'LineWidth', 1.5);
% 
% % loglog(NzVals, 2.5e-5*(NzVals .^ 3), '--', 'Color', c_green, 'LineWidth', 1.5);
% xlabel('dt'); ylabel('L_1 error'); title('Velocity error plot');
% legend('L_1', 'L_1 (n)', 'L_1 (u)', 'L_1 (T_a)', 'Order 2'); 
...
%     disp(ERRORS(6 + j, :)');
%     disp(log2(ERRORS(6 + j, 1:end-1)./ERRORS(6 + j, 2:end))');
% end
...
function [] = PlotF(f_vals_low_rank, Xmat, Rmat, Zmat2, dv_perp, Nz, fig_num)
    Nx = size(Xmat, 1);
    F = zeros(Nx, Nz);
    for i = 1:Nx
        f = f_vals_low_rank{i,1}*f_vals_low_rank{i,2}*f_vals_low_rank{i,3}';
...
% we don't use this method because we split up the IMEX222 fluid solver
