...
% Reference meshes for error calculation
% NOTE: Need to ensure that if you are testing accuracy in V_perp (radial) accuracy, the
...
for nvIndex = 1:numel(Nvvals)
    ...
    % time-stepping method: 1=B.Euler, 2=IMEX222
    method = '2';
    tolerance = 1e-8;
    ...
    Vr_max = 8; Vr_min = 0;
    Vz_max = 8; Vz_min = -8;
    interval = [x_min, x_max, Vr_min, Vr_max, Vz_min, Vz_max]; % 1D in x, 2D in v (x interval, r interval, z interval)
    ...
    for i = 1:Nx
        ...
        [~, Rmat_ref, Zmat_ref, ~, dr_ref, dz_ref] = GetRZ(Nx, Nr_ref, Nz_ref, interval);
    ...
    ERRORS(1, nvIndex) = dx*sum(error)/((x_max - x_min)*(Vr_max - Vr_min)*(Vz_max - Vz_min));
    ERRORS(2, nvIndex) = dx*sum(abs(n_vals - soln_n))/(x_max - x_min);
    ERRORS(3, nvIndex) = dx*sum(abs(u_para - soln_u))/(x_max - x_min);
    ERRORS(4, nvIndex) = dx*sum(abs(Ta_vals - soln_Ta))/(x_max - x_min);
    ERRORS(5, nvIndex) = linf_error;
end

[~, Rmat_ref, Zmat_ref, dx_ref, dr_ref, dz_ref] = GetRZ(Nx, Nr_ref, Nz_ref, interval);
zvals_ref = Zmat_ref(1, :)';
[Xmat_ref, Zmat_ref2] = meshgrid(xvals, zvals_ref); Xmat_ref=Xmat_ref'; Zmat_ref2=Zmat_ref2'; % for plotting only


PlotF(soln, Xmat_ref, Rmat_ref, Zmat_ref2, dr_ref, Nz_ref, 1);
title(sprintf('Exact solution at time t=%s', num2str(tf)));

PlotF(f_vals_low_rank, Xmat, Rmat, Zmat2, dr, Nz, 2);
